...
\subsection{物理网络显示图}
物理网络的拓扑显示如图\ref{fig:physicalnet}所示，由图可以看出，租户可以进行物理网络的初始化，力导向图可以对拓扑中的元素进行自动布局。在次基础上，租户可以进行全局网络可用带宽的测量，测量数据会在相应的链路之上进行显示。通过对链路的点击选取，在输入框输入租户自由控制器的IP地址，可以实现vSDN网络的创建，创建的虚拟网路由租户自由的控制器进行集中管控。除此之外，租户以可以进行删除虚拟网络的操作。
